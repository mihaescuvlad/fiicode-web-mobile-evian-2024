...
def compute_similarity(product1, product2, user_allergens=[]):
    ...
    similarity_score = (
        0.7 * numeric_similarity + 0.3 * ingredients_similarity - allergen_penalty
    )
...
def recommend_products(user_basket, all_products, user_allergens=[], N=9):
    ...
    for available_product in available_products:
        similarity_scores = [
            compute_similarity(user_product, available_product, user_allergens)
            for user_product in user_basket
        ]
