class Post {
  ...
  #Toggleable = {
    ...
    set active(value) {
      ...
      const [toAdd, toRemove] =
        value === this.shown
          ...
          : [this.inactiveClasses, this.activeClasses];
    ...
    get active() {
      if (
        this.activeClasses.every((cls) => this.element.classList.contains(cls))
      )
        return this.shown;
      if (
        this.inactiveClasses.every((cls) =>
  ...
  #upvote = {
    ...
    id: "upvote",
    className: "uparrow",
    activeClasses: ["mdi-thumb-up", "text-primary-400"],
    inactiveClasses: ["mdi-thumb-up-outline"],
    weight: 1,
    valueOf: function () {
      return this.id;
    },
  ...
  #downvote = {
    ...
    id: "downvote",
    className: "downarrow",
    activeClasses: ["mdi-thumb-down", "text-primary-400"],
    inactiveClasses: ["mdi-thumb-down-outline"],
    weight: -1,
    valueOf: function () {
      return this.id;
    },
  ...
  get vote() {
    if (this.#upvote.active === this.#upvote.shown) return this.#upvote;
    if (this.#downvote.active === this.#downvote.shown) return this.#downvote;
  ...
  set vote(vote) {
    ...
    const updateGUI = () => {
      const impact =
        (vote ? vote.weight : 0) - (this.vote ? this.vote.weight : 0);
    ...
    if (vote === null) {
      $.ajax({
        ...
        complete: (_, status) => {
          if (status !== "nocontent") {
            ErrorNotifier.get.show(
              "Failed to remove vote, please try again later"
            );
          } else {
            updateGUI();
          }
      ...
      $.ajax({
        ...
        complete: ({ responseJSON: res }, status) => {
          if (status !== "success") {
            if (res && res.message) ErrorNotifier.get.show(res.message);
            else
      ...
      complete: ({ status }) => {
        if (status >= 200 && status < 300) {
          ...
          if (status == 401)
            ...
            ErrorNotifier.get.show(
              "Failed to report post, please try again later"
            );
        }
      },
    });
